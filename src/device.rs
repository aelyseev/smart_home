...
impl Device {
    pub fn create_plug(name: &str, capacity: u16) -> Self {
        Device::SmartPlug(SmartPlug::new(name, capacity))
    }

    pub fn create_thermometer(name: &str, temperature: u16) -> Self {
...
mod test {
    ...
    fn create_plug_device() {
        let d = Device::create_plug("name", 120);
        match d {
    ...
    fn create_plug_thermometer() {
        let d = Device::create_thermometer("name", 34);
        match d {
